...
export const getSavedProduct = async (req: Request, res: Response) => {
  try {
    const getSavedProduct = await savedProductModel
      .find()
      .populate("userId")
      .populate("products");
...
export const getSavedProductById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const getSavedProductById = await savedProductModel
      .findById(id)
      .populate("userId")
      .populate("products");
...
export const updateSavedProduct = async (req: Request, res: Response) => {
  try {
    ...
    const updateSavedProduct = await savedProductModel.findByIdAndUpdate(
      orderId,
      req.body
    );
